...
def compute_pnl_for_strategy(strategy_id: int, window: str | None = None) -> pd.DataFrame:
    ...
    directional = df[df[col].isin(["long", "short"])].copy()
...
def compute_pnl_strategy1_combined() -> pd.DataFrame:
    ...
    if df.empty:
        return df
    return df.sort_values("entry_date_T").reset_index(drop=True)
...
def strategy_metrics(trades: pd.DataFrame) -> dict:
    ...
    # Dollar-weighted portfolio return: reflects conviction-based sizing
    total_pnl = float(trades["net_pnl_usd"].astype(float).sum()) if "net_pnl_usd" in trades.columns else 0.0
    portfolio_ret = total_pnl / CAPITAL_USD

    # Max drawdown using dollar equity curve (respects position sizing)
    sorted_trades = trades.sort_values("entry_date_T")
    if "net_pnl_usd" in sorted_trades.columns:
        equity_usd = CAPITAL_USD + sorted_trades["net_pnl_usd"].astype(float).cumsum()
        running_max = equity_usd.cummax()
        dd = (equity_usd - running_max) / running_max
    else:
        equity_usd = (1.0 + sorted_trades["net_return_pct"].astype(float)).cumprod()
        running_max = equity_usd.cummax()
        dd = (equity_usd - running_max) / running_max
    ...
    return {
        ...
        "portfolio_return_pct": float(portfolio_ret),
        "total_net_pnl_usd": float(total_pnl),
