...
MODEL_ID = "Qwen/Qwen2.5-72B-Instruct"  # HuggingFace Inference API
...
def run(*, agent1_out: Agent1Out, target_quarter_end: date,
    ...
    if cons.get("median_usd_m") is not None and cons.get("n_analysts", 0) >= 1:
        ...
        if trailing_avg > 1.0:
            ...
            # trailing_avg ≤ 1: pads activated from dormant baseline (trailing ≈ 0).
            # Relative change is undefined; use pad activity composition as proxy:
            #   signal = (newly_active - idle) / total_pads  ∈ [-1, +1]
            # This captures directional signal when a company goes from
            # 0 active pads to N active (strong bullish) or vice versa.
            total_pads = (agent1_out.n_newly_active
                          + agent1_out.n_continuously_active
                          + agent1_out.n_idle)
            if total_pads > 0:
                raw = ((agent1_out.n_newly_active + agent1_out.n_continuously_active
                        - agent1_out.n_idle) / total_pads)
                # Cap at ±0.5: proxy signal only justifies modest divergence,
                # not strong_beat/miss (those require the primary relative-delta formula).
                drilling_signal = max(-0.5, min(0.5, raw))
            else:
                drilling_signal = 0.0
