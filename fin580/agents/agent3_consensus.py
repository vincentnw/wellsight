...
def run(
    *,
    ...
    # high: strong divergence signal (|div|>15%) with solid analyst panel (n>=10
    # and reasonable consensus agreement, dispersion < 20%); OR strong signal
    # with very tight analyst agreement (dispersion < 5%).
    if div_class in ("strong_beat", "strong_miss") and n_analysts >= 10 and dispersion_ratio < 0.20:
        conf = "high"
    elif n_analysts >= 10 and dispersion_ratio < 0.05:
        conf = "high"
    elif n_analysts >= 5:
        conf = "medium"
    else:
