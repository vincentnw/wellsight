...
def test_short_trade_profits_on_price_decline():
    # Short 10% of $1M = $100k. Price falls from 100→95 → gross +$5k, costs $300.
    pnl = compute_trade_pnl(
        entry_price=100.0, exit_price=95.0,
        size_pct=0.10, direction="short",
        capital_usd=1_000_000, cost_bps=30,
    )
    assert 4500 <= pnl["net_pnl_usd"] <= 4900, pnl
    assert pnl["gross_return_pct"] > 0


def test_short_trade_loses_on_price_rise():
    # Short 10% of $1M. Price rises 100→105 → gross -$5k, costs $300.
    pnl = compute_trade_pnl(
...
def test_short_and_long_are_symmetric():
    # Long profits = Short losses (mirror P&L, same absolute magnitude).
    long_pnl = compute_trade_pnl(
        entry_price=100.0, exit_price=110.0,
        size_pct=0.10, direction="long",
        capital_usd=1_000_000, cost_bps=0,
    )
    short_pnl = compute_trade_pnl(
        entry_price=100.0, exit_price=110.0,
        size_pct=0.10, direction="short",
        capital_usd=1_000_000, cost_bps=0,
    )
    assert abs(long_pnl["gross_pnl_usd"] + short_pnl["gross_pnl_usd"]) < 1e-6
