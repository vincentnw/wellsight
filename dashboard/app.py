...
def _latest_run_dir(year_prefix: str, fallback: str) -> Path:
    """Return the most recent run dir matching the year prefix, else fallback."""
    candidates = sorted(
        [d for d in RUNS_DIR.iterdir()
         if d.is_dir() and f"strategy1-{year_prefix}" in d.name],
        reverse=True,
    )
    return candidates[0] if candidates else RUNS_DIR / fallback


HEADLINE_RUN = _latest_run_dir("2024Q1", "2026-05-04-strategy1-2024Q1_2024Q4-target-realsar-v2_5")
WINDOW_RUNS = {
    "2019": _latest_run_dir("2019Q1", "2026-05-04-strategy1-2019Q1_2019Q4-target-realsar-v2_5"),
    "2020": _latest_run_dir("2020Q1", "2026-05-04-strategy1-2020Q1_2020Q4-target-realsar-v2_5"),
    "2021": _latest_run_dir("2021Q1", "2026-05-04-strategy1-2021Q1_2021Q4-target-realsar-v2_5"),
    "2022": _latest_run_dir("2022Q1", "2026-05-04-strategy1-2022Q1_2022Q4-target-realsar-v2_5"),
    "2023": _latest_run_dir("2023Q1", "2026-05-04-strategy1-2023Q1_2023Q4-target-realsar-v2_5"),
    "2024": HEADLINE_RUN,
}
...
st.sidebar.markdown(
    ...
    - [5. All Trades (Long + Short)](#section-5)
...
c3.metric("Trades fired", f"{len(all_trades)}")
if len(all_trades):
    wins_6y = (all_trades.net_return_pct > 0).sum()
    c4.metric("Hit rate", f"{wins_6y/len(all_trades):.0%}")
    total_pnl_6y = float(all_trades["net_pnl_usd"].sum()) if "net_pnl_usd" in all_trades.columns else 0.0
    c5.metric(
        "Portfolio return",
        f"{total_pnl_6y/1_000_000*100:+.2f}%",
        help=f"Total net P&L ${total_pnl_6y:+,.0f} on $1M capital (conviction-sized)",
    )
    c6.metric(
        "Total net P&L",
        f"${total_pnl_6y:+,.0f}",
...
if all_cells.empty:
    ...
    if cell_row["decision"] == "long":
    ...
    elif cell_row["decision"] == "short":
        st.warning(
            f"📉 **SHORT** — {cell_row['conviction_tier']} conviction, "
            f"{cell_row['final_size_pct']:.0%} size  ·  "
            f"{n_active_preview}/{n_pads_preview} pads active  ·  "
            f"divergence {div_preview:+.1f}% vs consensus"
        )
        ...
        if err:
            ...
            st.info(
                f"🚫 **NO TRADE** — {n_active_preview}/{n_pads_preview} pads active "
                f"({div_preview:+.1f}% divergence; `modest_beat`/`strong_beat` required "
                "for long; `modest_miss`/`strong_miss` required for short)"
            )
        else:
    ...
    with st.expander("⚖️ **Step 3 — Agent 3 (Consensus Comparison)**"):
        ...
        if a3 and "error" not in a3:
            ...
            if a3["divergence_class"] == "in_line":
                st.warning(
                    f"Divergence class is **in_line** → "
                    "Agent-3 gate fires → **no_trade**. Agents 4 and 5 do not run."
                )
            elif a3["divergence_class"] in ("modest_beat", "strong_beat"):
                st.success(
                    f"Divergence class is **{a3['divergence_class']}** → cell proceeds "
                    "to Agents 4 and 5 (long candidate)."
                )
            elif a3["divergence_class"] in ("modest_miss", "strong_miss"):
                st.info(
                    f"Divergence class is **{a3['divergence_class']}** → cell proceeds "
                    "to Agents 4 and 5 (short candidate)."
                )
        else:
...
# =============================================================================
st.header("5. Every trade the system fired (2019-2024)", anchor="section-5")
...
if len(all_trades) == 0:
    ...
    n_longs = (all_trades["direction"] == "long").sum() if "direction" in all_trades.columns else len(all_trades)
    n_shorts = (all_trades["direction"] == "short").sum() if "direction" in all_trades.columns else 0
    _n_high   = (all_trades.get("conviction_tier", pd.Series()) == "high").sum() if "conviction_tier" in all_trades.columns else 0
    _n_medium = (all_trades.get("conviction_tier", pd.Series()) == "medium").sum() if "conviction_tier" in all_trades.columns else len(all_trades)
    _n_low    = (all_trades.get("conviction_tier", pd.Series()) == "low").sum() if "conviction_tier" in all_trades.columns else 0
    st.caption(
        f"**{len(all_trades)} trades** ({n_longs} long, {n_shorts} short) across "
        f"**{all_trades.window.nunique()} windows** — "
        f"🔴 {_n_high} high, 🟡 {_n_medium} medium, 🟢 {_n_low} low conviction. "
        "Each trade enters at T-14 days before the earnings call and exits 2 trading "
        "days strictly after the announcement."
    )

    # Conviction sizing info box
    import os as _os
    _sz_high   = float(_os.environ.get("FIN580_SIZE_HIGH",   "0.15"))
    _sz_medium = float(_os.environ.get("FIN580_SIZE_MEDIUM", "0.10"))
    _sz_low    = float(_os.environ.get("FIN580_SIZE_LOW",    "0.05"))
    st.info(
        f"**Conviction-based position sizing** — each trade risks a fraction of the $1M capital base.  "
        f"🔴 High conviction → **{_sz_high:.0%}** (${1_000_000*_sz_high:,.0f})  |  "
        f"🟡 Medium → **{_sz_medium:.0%}** (${1_000_000*_sz_medium:,.0f})  |  "
        f"🟢 Low → **{_sz_low:.0%}** (${1_000_000*_sz_low:,.0f}).  "
        f"Override via `FIN580_SIZE_HIGH / _MEDIUM / _LOW` env vars."
    )

    # Per-trade table — one row per trade, sorted chronologically
    sorted_t = all_trades.sort_values("entry_date_T").reset_index(drop=True)
    _dir_col = sorted_t["direction"] if "direction" in sorted_t.columns else pd.Series(["long"] * len(sorted_t))
    _tier_col = sorted_t["conviction_tier"] if "conviction_tier" in sorted_t.columns else pd.Series(["medium"] * len(sorted_t))
    _tier_icon = {"high": "🔴 High", "medium": "🟡 Med", "low": "🟢 Low", "none": "—"}
    table = pd.DataFrame({
        ...
        "Direction": _dir_col.map(lambda d: "📈 Long" if d == "long" else "📉 Short"),
        "Conviction": _tier_col.map(lambda t: _tier_icon.get(str(t), "🟡 Med")),
    ...
    from fin580.inference.pnl import strategy_metrics, CAPITAL_USD
    ...
    port_ret = m.get("portfolio_return_pct") or 0.0
    total_pnl = m.get("total_net_pnl_usd") or 0.0
    mc[1].metric(
        "Portfolio return",
        f"{port_ret*100:+.2f}%",
        help=f"Total net P&L ${total_pnl:+,.0f} ÷ ${CAPITAL_USD:,.0f} starting capital. "
             "Reflects conviction-based sizing (high=25%, medium=10%, low=5%).",
    )
    mc[2].metric(
        "Mean per-trade return",
        f"{m['mean_net_return_pct']*100:+.2f}%",
        help="Unweighted average of individual trade price returns. Independent of sizing.",
    )
    ...
    mc[4].metric("Max drawdown", f"{(m['max_drawdown_pct'] or 0)*100:+.2f}%",
                 help="Largest peak-to-trough drop in dollar equity curve.")

    # Cumulative equity curve — dollar P&L (reflects conviction-based sizing)
    sorted_t = sorted_t.sort_values("entry_date_T").reset_index(drop=True)
    sorted_t["cum_equity"] = CAPITAL_USD + sorted_t["net_pnl_usd"].astype(float).cumsum()
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=[sorted_t.iloc[0]["entry_date_T"]] + list(sorted_t.entry_date_T),
        y=[CAPITAL_USD] + list(sorted_t.cum_equity),
        mode="lines+markers", marker_size=10,
        line=dict(color="steelblue", width=3),
        name="Portfolio equity ($)",
        hovertemplate="%{x}<br>Equity: $%{y:,.0f}<extra></extra>",
    ))
    fig.add_hline(y=CAPITAL_USD, line_dash="dash", line_color="gray",
                  annotation_text="Starting capital $1M")
    fig.update_layout(
        title="Portfolio equity curve 2019-2024 (conviction-weighted, $1M capital)",
